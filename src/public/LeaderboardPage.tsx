      ...
      {data && (
        <>
          ...
          <section className="leaderboard-section leaderboard-analysis" aria-labelledby="analysis-title">
            <SectionHeading label="Aggregate research report" title="Evidence interpretation" id="analysis-title" />
            {data.latestReport?.status === 'complete'
              ? <div className="analysis-copy">
                  <p>{data.latestReport.title ?? 'Public evidence report'} covers {data.latestReport.completePairs.toLocaleString()} matched questions across {data.latestReport.modelCount.toLocaleString()} models with dimension charts and full stored evidence.</p>
                  <a className="text-link" href={`/api/public/reports/${data.latestReport.id}.html`}>Read the full research report <span aria-hidden="true">→</span></a>
                  <small>Generated {data.latestReport.completedAt ?? data.latestReport.createdAt}</small>
                </div>
              : data.reportPending
                ? <p className="muted">A dimension-scored aggregate report is being generated from the public evidence collected so far.</p>
                : <p className="muted">A full dimension-scored report is generated automatically once enough mature matched questions accumulate across multiple models.</p>}
          </section>
