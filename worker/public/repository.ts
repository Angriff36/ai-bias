...
function mapCatalogEvidenceRow(row: Record<string, unknown>): PublicEvidenceItem {
  return {
    ...
    prompt: '', response: '', latencyMs: 0, statusCode: 0, status: s(row.status) as 'ok' | 'error',
...
const catalogEvidenceSelect = `SELECT id, run_id, pair_index, run_index, question, variant_key, provider, model_id, status, received_at
  FROM public_evidence`
