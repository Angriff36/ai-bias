...
export class PublicRepository {
  ...
  async publish(raw: PublicSubmission, receivedAt: string): Promise<{
    ...
    for (const record of submission.records) {
      statements.push(this.db.prepare(`INSERT INTO public_evidence
        (id, run_id, pair_index, run_index, question, variant_key, variant_label, provider, model_id, prompt, response, latency_ms, status_code, status, error_message, truncated, evidence_sha256, classification, received_at)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`)
        .bind(crypto.randomUUID(), runId, record.pairIndex, record.runIndex, record.question ?? null, record.variantKey,
          record.variantLabel, record.provider, record.modelId, record.prompt, record.response, record.latencyMs,
          record.statusCode, record.status, record.errorMessage ?? null, record.truncated ? 1 : 0, record.sha256,
          classifyPublicEvidence(record), receivedAt))
    }
  ...
  async getLeaderboard(modelLimit = 50, recentLimit = 40): Promise<PublicLeaderboard> {
    ...
    const evidenceRows = (await this.db.prepare(`SELECT id, run_id, pair_index, run_index, question, variant_key, variant_label,
      ...
      FROM public_evidence ORDER BY received_at DESC, run_id DESC, pair_index ASC, variant_key ASC LIMIT ?`).bind(recentLimit).all()).results ?? []
    ...
    const recentEvidence: PublicEvidenceItem[] = evidenceRows.map((row) => ({
    ...
    return {
      totals: { runs: n(totals?.runs), responses: n(totals?.responses), completePairs: n(totals?.complete_pairs), models: n(totals?.models) },
      models,
