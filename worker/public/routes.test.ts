...
function dependencies() {
  return {
    ...
    reportRepository: {
      ...
      evaluateGlobalReportAfterPublish: vi.fn(async () => ({ kind: 'claimed', report: { id: 'global-cohort', scope: 'global', status: 'pending', title: null, responseCount: 0, completePairs: 0, modelCount: 0, createdAt: 'now', completedAt: null } })),
...
describe('public API routes', () => {
  it('evaluates global report maturity after publish instead of raw response watermarks', async () => {
    ...
    expect(deps.reportRepository.evaluateGlobalReportAfterPublish).toHaveBeenCalledTimes(1)
    expect(deps.scheduleReport).toHaveBeenCalledWith('global-cohort')
  })
  ...
  it('claims eligible reports, lists them, and serves safe standalone HTML', async () => {
    ...
    expect(await html?.text()).toContain('Dimension charts')
  })
